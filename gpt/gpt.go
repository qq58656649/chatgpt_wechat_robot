...
type ChoiceItem struct {
	Message      Message `json:"message"` // 更新为 Message 结构体
...
type ChatGPTRequestBody struct {
	Model       string  `json:"model"`
	Messages    []Message `json:"messages"` // 更新为消息数组
...
func Completions(msg string) (string, error) {
	...
	if gptResponseBody != nil && len(gptResponseBody.Choices) > 0 {
		reply = gptResponseBody.Choices[0].Message.Content // 更新为访问新的结构
	}
	return reply, nil
}
...
func httpRequestCompletions(msg string, runtimes int) (*ChatGPTResponseBody, error) {
	...
	requestBody := ChatGPTRequestBody{
		Model:            cfg.Model,
		Messages:         []Message{{Role: "user", Content: msg}}, // 更新为消息数组
	...
	req, err := http.NewRequest(http.MethodPost, "https://api.openai.com/v1/chat/completions", bytes.NewBuffer(requestData)) // 更新为新接口 URL
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("json.Unmarshal responseBody error: %v", err)
	}
